...
- **最終報告は要点のみ**: 変更一覧はコミット1件につき1行、検証結果は green/red と件数、レビューは収束経過1行（例: R1 must4+should2 → R2 must2 → R3 ゼロ）、実測フッター1行。全体でおおむね15行以内に収め、詳細は direction の完了記載に転記する
...
7. must-fix / should-fix は完了済みの該当 implementer へ `followup_task` で差し戻す（軽微ならリーダー直対応）。修正後は再レビューを起動し、**指摘が無くなるまでレビュー→修正を繰り返す**。5ラウンド収束しない、または同一タスクの差し戻しが3回を超えたら停止し、未解決指摘とともにユーザーへ報告する

...
- direction を作らない小タスクでも、実装完了後のレビューは行う: `team-impl` のプレレビュー節と同じ手順（同ファミリー最上位モデルの reviewer エージェントで must-fix / should-fix がゼロに収束するまでプレレビューし、収束してから `cross-review` を起動する。cross-review も must-fix / should-fix がゼロになるまでループする）に従う。
...
2. 状態行の下に実測フッターを1行追記する（team-impl の完了報告があればそれを転記。数値は概算でよい）:
   ...
   `実測: レビュー<N>R（R1 must<N>+should<N>） / 差し戻し<N> / リーダー直修正<N> / 追補<N>（契約<N>） / 逸脱: <スキル手順と実態がずれた点。無ければ「なし」>`

   R1 指摘件数は計画品質の直接指標として必ず含める（R 数だけでは検証設計・横断関心の振り分けが効いたか判定できない）。

...
- 出力は `{ sessions: [...], totals: {...} }` の JSON。各 session は `wallClockMs`（壁時計）・`userWaitMs`（turn 外の応答待ち。人間入力・teammate/task 通知とも）・`activeMs`（実働）・`breakdown`（`llmGenerationMs` / `methodOpsMs` / `toolExecutionMs` / `delegationWaitMs` / `unattributedMs` の排他パーティション）・`turnDurationCheckMs` / `turnWindowActiveMs` / `turnWindowCheckMs`（Claude）または `durationCheckMs` / `mcpDurationMs`（Codex）・`tokens`・`loops`・`quality` を持つ。分は呼び出し側で四捨五入する
...
- Claude の検算は **clean turn 窓限定**: `turn_duration` の窓 `[ts−durationMs, ts]` は turn 開始プロンプトにアンカーされるが、窓内部にプロンプトを含む turn（teammate 往復・steering を含む長大 turn）の `durationMs` は turn 内の応答待ちを含むため検算から除外し、内部プロンプトの無い clean 窓だけで `turnWindowActiveMs`（窓内 active）と `turnWindowCheckMs`（窓 durationMs 合計）を比較する。clean 窓が無いログは検算不能（null）で、`turnDurationCheckMs`（全 `turn_duration` 合算）は参考値として残る
...
- **壁時計と実働を分けて**示し、ユーザー待ちを除いた実働（`activeMs`）の内訳（`breakdown`）を大きい順に報告する。検算の乖離（Claude は `turnWindowActiveMs` 対 `turnWindowCheckMs`、Codex は `activeMs` 対 `durationCheckMs`）も添える

...
- **Show（デフォルト）**（下位2レーンの基準に触れないすべて）: 直接実装 → 機械ゲート → プレレビュー**1回**（Claude Code は `dev-method-claude:reviewer` agent、Codex は `spawn_agent` の `agent_type="reviewer"`・`fork_turns="none"` で起動。**must-fix のみ即対応**、should-fix / nit は蓄積して follow-up 1バッチ）。cross-review なし。UI に見える変更を含むなら `scenario-kit smoke` を機械ゲートに加え、実測に `smoke <PASS|FAIL n件|評価不能|対象外>` を記録する（UI に見える変更が無ければ `対象外`、`未整備` は使わない）

...
他スキルが観点ブロックのみを流用する場合（team-impl のプレレビュー等、ファイル出力しない報告契約の実行）は「## 観点」節のみを使い、「## 出力形式」節は含めない（報告形式は流用先スキルの契約に従う）。この流用規定の正本はこのファイルとする。
...
変更が検知器（テスト基盤・検証スクリプト・パーサ・品質ゲート）に該当するかどうかを diff から判定し、該当する場合のみ、渡された故意ずれ検体を実際に実行して期待どおりの合否になるか確認する（このケースに限り検証コマンドの実行を許可する）。流用先の契約（team-impl プレレビュー等）がコマンド実行を禁じる場合はこの実行指示を読み替え、渡された実行証跡と検体の静的照合に縮退する（実行証跡が無い失敗クラスは should-fix として報告し、自分の権限を広げて追加検証を実行しない）。検体の所在と期待合否は重点観点または検証証跡の差し込みで渡される。該当しない場合はこのブロックを無視してよい。

...
| プラグイン | 中身 | インストール先 |
| --- | --- | --- |
| `dev-method` | 共通スキル: `direction` / `setup` / `cross-review` / `method-check` / `playwright-cli` / `scenario-kit` / `plugin-release` / `bug-diagnosis` / `keel` / `service-provisioning` | Claude Code / Codex 両方 |
...
| `service-provisioning` | サービス公開までのプロビジョニングを API で実行する: DNS・ホスティング・メール窓口の設定を、既定 dry-run・設定値の照合による冪等・自動化できない工程の明示的な受け渡しで通す |

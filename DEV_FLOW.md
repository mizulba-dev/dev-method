...
入口はつねに「レーン判定」。**レーンは変更の爆発半径だけで決まり、direction を書くかどうかとは独立**（direction は設計合意の道具であって、書いても実装工程は重くならない）。実行・入出力・影響がローカル環境内で完結し、かつ非公開・非配布のツールは通常判定より先に Ship / Show へ限定する。出荷後の逸脱・バグは friction ログに落ち、たまったら手法そのものを改訂して配布し直す — 手法自体が改善ループの対象になっている。

```mermaid
flowchart TD
  REQ["実装・修正の依頼"] --> JUDGE{"レーン判定<br/>（爆発半径のみ・1行宣言）"}
  JUDGE -->|"ローカル完結・非公開/非配布<br/>× 挙動非変更"| SHIP["<b>Ship</b><br/>機械ゲートのみ"]
  JUDGE -->|"ローカル完結・非公開/非配布<br/>× 挙動変更"| SHOW["<b>Show</b><br/>プレレビュー1回"]
  JUDGE -->|"通常判定: 挙動に触れない"| SHIP
  JUDGE -->|"通常判定: デフォルト"| SHOW
  JUDGE -->|"通常判定: 高リスク基準 /<br/>検知器の新設・変更"| SIGN["<b>Sign</b><br/>pre + cross 各1回"]
  JUDGE -->|"通常判定: 不可逆 × 外部影響"| SEAL["<b>Seal</b><br/>direction フルパイプ"]
  SHIP --> OUT["出荷"]
  SHOW --> OUT
  SIGN --> OUT
  SEAL --> OUT
...
> Show は原義（Ship/Show/Ask の「マージ後の事後レビュー」）と異なり、ここでは「出荷前の1回レビュー」を指す再定義。ローカル例外に該当しない検知器（テスト基盤・検証スクリプト・パーサ・品質ゲート）の新設・変更は規模によらず Sign 以上 — 検知器自身の false green は静的レビューで見抜きにくいため、実データ×不変式の機械的敵対者と異ベンダーの独立実行検証を必須にしている。

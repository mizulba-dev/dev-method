...
flowchart TD
  ...
  Q2 -->|"Yes"| SEAL["<b>Seal</b><br/>個人開発では年数回が正常頻度"]
...
> direction を書いた作業の完了記載では、レーンを問わず `method-check` の実測が必須（実働・手法運用の分数は session-metrics.mjs の実測 JSON からのみ確定し、品質条件を満たさなければ「欠測」として省略する — 体感の自己申告はしない）。
